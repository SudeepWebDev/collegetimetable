...
class TimetableEntry(models.Model):
    timetable = models.ForeignKey(Timetable, on_delete=models.CASCADE, related_name='entries', default=None)
    day = models.CharField(max_length=100, choices=DAYS_CHOICES, verbose_name="Day")
    lecture_type = models.ForeignKey(Lecture, on_delete=models.CASCADE, verbose_name="Lecture Type")
    ...
    faculty_2 = models.ForeignKey(Faculty, on_delete=models.CASCADE, null=True, blank=True,
                                  related_name='faculty_2', verbose_name="Faculty 2 (for lab)")

    def __str__(self):
        return f"{self.day} - {self.subject} - {self.timing} - {self.faculty}"

class Meta:
    unique_together = ['day', 'lecture_type', 'subject', 'room', 'timing', 'faculty', 'faculty_2']